...
public class RotateImage : MonoBehaviour
{
    ...
    void CheckInitialSelection()
    {
        ...
        int closestIndex = 0;
        float smallestDiff = 999f;

        for (int i = 0; i < images.Count; i++)
        {
            float angle = currentAngle + startingAngleOffset + i * angleStep;
            float diff = Mathf.Abs(Mathf.DeltaAngle(angle, frontAngle));
    ...
    public RectTransform GetImageAtDotSlot()
    {
        float angleStep = 360f / images.Count;
        int closestIndex = 0;
        float smallestDifference = 999f;
